...
- `Status / Select a Display`: choose what the Mac should stream.
...
1. Open the display picker in the `Select a Display` area.
...
- `Source Name`: the published stream name
- `Bandwidth Mode`: Best, Better, Balanced, Low Bandwidth, or Custom
...
- Turn `Battery Alerts`, `Disconnect Alerts`, and `Reconnect Alerts` on or off
...
- Turn `Capture One Integration` on or off
- Decide which Capture One controls are allowed from iPad
- Review or change the Capture One target app bundle
- Optionally enable the legacy BetterDisplay rotation fallback if your build exposes it
...
1. `Settings > Integrations > Capture One Integration` is turned on.
